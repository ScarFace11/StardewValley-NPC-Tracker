...
{
    /// <summary>
    /// "Settings" tab: Hotkeys, Time filter, Appearance with SDV-style controls.
    /// </summary>
    public partial class TrackingMenu
    {
        ...
        private int AppearanceSectionY => TimeRowY + 44 + 8 + 22 + 40;
        ...
        // ── Bottom buttons ───────────────────────────────────────────────────────────
        private Rectangle SaveSettingsBtnRect() =>
            new Rectangle(BX + PAD, BY + 612, 160, 34);
        private Rectangle ResetSettingsBtnRect() =>
            new Rectangle(BX + BOX_W / 2 - 30, BY + 612, 180, 34);
        ...
        private void DrawSettingsTab(SpriteBatch b)
        {
            ...
            var endSz = Game1.smallFont.MeasureString("26:00");
            Utility.drawTextWithShadow(b, "26:00", Game1.smallFont,
        ...
        private void DrawSaveSettingsButton(SpriteBatch b)
        {
            var rect = SaveSettingsBtnRect();
            bool hov = rect.Contains(Game1.getMouseX(), Game1.getMouseY());

            drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60),
                rect.X, rect.Y, rect.Width, rect.Height,
                hov ? new Color(180, 160, 60) : new Color(140, 120, 50), 0.85f, false);

            string label = T("settings.save");
            ...
            if (hov) _hoverText = T("settings.save.tip");
        }
        ...
        private void DrawResetSettingsButton(SpriteBatch b)
        {
            ...
            string label = T("settings.restoreDefaults");
            var sz = Game1.smallFont.MeasureString(label);
            Utility.drawTextWithShadow(b, label, Game1.smallFont,
                new Vector2(rect.X + (rect.Width - sz.X) / 2f,
                            rect.Y + (rect.Height - sz.Y) / 2f),
            ...
            if (hov) _hoverText = T("settings.restoreDefaults.tip");
        }
        ...
        private void ClickSettings(int x, int y, bool playSound)
        {
            ...
            // Save button
            if (SaveSettingsBtnRect().Contains(x, y))
            {
                SaveSettings();
                if (playSound) Game1.playSound("smallSelect");
                return;
